		...
		<main class="contenido">
			...
			<section class="registroSeccion">
				<div class="registroContenido">
					...
					<form class="formularioLogin" action="/Proyecto-PInfra/SvEditarUsuario" method="POST">
						<div class="formularioContenido">
							<label>Nombres <input type="text" name="nombres"
								placeholder="Ingrese sus nombres..." value="<%=oUsuarioAEditar.getNombres()%>">
							</label> 
							<label>Apellidos <input type="text" name="apellidos"
								placeholder="Ingrese sus apellidos..." value="<%=oUsuarioAEditar.getApellidos()%>">
							</label> 
							<label>Cédula <input type="number" name="cedula"
								placeholder="Ingrese su cédula..." value="<%=oUsuarioAEditar.getDocumento()%>" disabled>
							</label> 
							<label>Teléfono <input type="number" name="telefono"
								placeholder="Ingrese su teléfono..." value="<%=oUsuarioAEditar.getTelefono()%>">
							</label> 
							<label>Mail Personal <input type="text"	name="mailPersonal" 
								placeholder="Ingrese su mail personal..." value="<%=oUsuarioAEditar.getMailPersonal()%>">
							</label> 
							<label>Mail Institucional <input type="text" name="mailInstitucional"
								placeholder="Ingrese su mail institucional..." value="<%=oUsuarioAEditar.getMailInstitucional()%>" disabled>
							</label>
							<label>Fecha de Nacimiento 
								<input type="text" name="fechaNacimiento" placeholder="Ingrese su fecha de nacimiento..." value="<%=oUsuarioAEditar.getFechaNacimiento()%>">
							</label> 
							<label>Género 
							<select name="genero">
								...
								<option value="<%=oGenero.getNombre()%>" <%=opcionSeleccionada%>><%=oGenero.getNombre()%></option>
							...
							<label>ITR 
								<select name="itr">
									...
									for (Itr oItr : ServiceItr.listarItrs()) {
							            ...
							            String opcionSeleccionada = (oUsuarioAEditar != null && oItr.getNombre().equals(oUsuarioAEditar.getItr().getNombre())) ? "selected" : "";
									%>
							...
							<label>Departamento 
								...
								id="departamentoSelect" onchange="cargarLocalidades()">
									...
									for (Departamento oDepartamento : ServiceUbicacion.listarDepartamentos()) {
							            ...
							            String opcionSeleccionada = (oUsuarioAEditar != null && oDepartamento.getNombre().equals(oUsuarioAEditar.getDepartamento().getNombre())) ? "selected" : "";
									%>
									<option value="<%=oDepartamento.getNombre()%>" <%=opcionSeleccionada%>><%=oDepartamento.getNombre()%></option>
									<%
									}
									%>
								</select>
							...
							<label>Rol 
								<select name="rol" id="rolSelect" onchange="mostrarCampos()">
									...
									for (Rol oRol : ServiceRol.listarRoles()) {
							            ...
							            String opcionSeleccionada = (oUsuarioAEditar != null && oRol.getDescripcion().equals(oUsuarioAEditar.getRol().getDescripcion())) ? "selected" : "";
									%>
	
									<option value="<%=oRol.getDescripcion()%>" <%=opcionSeleccionada%>><%=oRol.getDescripcion()%></option>
							...
							<label id="areaLabel" style="display: none;">Área
								<select	name="area">
									<option selected>Seleccione su área</option>
									<%
									String opcionDeUsuario = Buscar.tutorFiltro(oUsuarioAEditar.getDocumento().toString(), "Documento").get(0).getArea().getDescripcion();
									System.out.println(opcionDeUsuario);
									System.out.println("--------------------------------------------------------------------");
									for (Area oArea : ServiceArea.listarAreas()) {
										
										// Verifica si el Area actual es igual al Area de oUsuarioAEditar
							            String opcionSeleccionada = (oUsuarioAEditar != null && oArea.getDescripcion().equals(opcionDeUsuario) ? "selected" : "");
									%>

									<option value="<%=oArea.getDescripcion()%>" <%=opcionSeleccionada%>><%=oArea.getDescripcion()%></option>
							...
							<label id="semestreLabel" style="display: none;">Semestre
							...
							<label id="generacionLabel" style="display: none;">Generación
